...
async function DeleteVoteQuestion(api_key, vote_id, data){
    try {
        const requestOptions = {
            method: 'DELETE',
            headers: {
...
async function CopyVote(api_key, vote_id){
    try {
        const requestOptions = {
            method: 'POST',
            headers: {
        ...
        let result = await fetch(process.env.API_URL + `/api/v1/votes/${vote_id}/copy`, requestOptions)
        let response = await result.json();
        return {
            ok: (result.status == 201)?true:false,
            data: response
